实验说明：在主机和Docker容器中分别运行相同的PyTorch批量推理任务，比较两者的性能差异。使用MobileNetV2模型进行100次推理，并记录各个阶段的时间消耗。

实验设置：
- 主机：不限制CPU和内存使用。
- Docker容器：限制使用2GB内存和2个CPU核心。
- torch 版本：2.5.1+cpu
- 模型：MobileNetV2，预训练权重来自torchvision。
...
瓶颈分析：
- `import torch` 会消耗大量时间
- Docker 容器还会有固定 400ms 左右的启动开销
